            ...
            <div id="important-dates-print">
                ...
                {/* Action Button */}
                 <div className="flex justify-end mb-4 no-print">
                     {sortedItems.length > 0 && <ActionButtons textToCopy="" elementIdToPrint="important-dates-print" />}
                 </div>
                ...
                <div className="space-y-3">
                    {sortedItems.map(item => (
                        <div key={item.id} className={`neumorphic-outset p-4 flex items-center gap-4 transition-all break-inside-avoid ${item.done ? 'opacity-60 bg-gray-50 dark:bg-gray-900' : 'bg-white dark:bg-gray-800'}`}>
                            ...
                            <button onClick={() => deleteItem(item.id)} className="w-10 h-10 rounded-full bg-red-100 text-red-500 hover:bg-red-500 hover:text-white flex items-center justify-center transition-colors no-print">
