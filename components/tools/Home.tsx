      ...
      <div className="space-y-4">
        {tools.map((tool) => (
          <div key={tool.key} className="relative flex items-center h-16 w-full cursor-pointer" onClick={() => onSelectTool(tool.key)}>
             {/* Icon */}
            <div className="absolute right-0 top-0 z-20 w-16 h-16 rounded-full bg-primary border-4 border-icon flex items-center justify-center text-icon shadow-md">
                <i className={`${tool.icon} text-3xl`}></i>
            </div>
             {/* Button */}
            <div className="absolute left-0 top-2 h-12 w-[calc(100%-3rem)] bg-primary rounded-full flex items-center justify-center text-icon shadow-md transition-transform transform hover:scale-[1.02]">
